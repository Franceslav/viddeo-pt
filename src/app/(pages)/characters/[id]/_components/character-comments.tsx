...
import CharacterCommentReplies from './character-comment-replies'

interface CharacterCommentsProps {
  characterId: string
}

const CharacterComments = ({ characterId }: CharacterCommentsProps) => {
  const { data: session } = useSession()
  const utils = trpc.useUtils()
  const [replyingTo, setReplyingTo] = useState<string | null>(null)

  const { data: comments, isLoading, error } = trpc.characterComment.getCharacterComments.useQuery({ characterId })

  const deleteComment = trpc.characterComment.deleteCharacterComment.useMutation({
    onSuccess: () => {
      ...
      <div className="text-center py-8">
        <div className="bg-yellow-100 border-2 border-yellow-400 rounded-lg p-6">
          ...
          <p className="text-yellow-700 mt-2">
            Будьте первым, кто оставит комментарий к этому персонажу!
          </p>
        </div>
      </div>
    )
  ...
  return (
    <div className="space-y-4">
      ...
      {comments.map((comment) => {
        const likes = comment.characterCommentLikes?.filter(like => like.type === 'like').length || 0
        const dislikes = comment.characterCommentLikes?.filter(like => like.type === 'dislike').length || 0
        const userLike = comment.characterCommentLikes?.find(like => like.userId === session?.user?.id)?.type as 'like' | 'dislike' | null || null
